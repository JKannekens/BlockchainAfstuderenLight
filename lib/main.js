...
class Blockchain {
    ...
    isChainValid() {
        for (let i = 1; i < this.chain.length; i++) {
            const currentBlock = new Block(this.chain[i].index, this.chain[i].timestamp, this.chain[i].data, this.chain[i].previousHash, this.chain[i].hash);
            const previousBlock = new Block(this.chain[i - 1].index, this.chain[i - 1].timestamp, this.chain[i - 1].data, this.chain[i - 1].previousHash, this.chain[i - 1].hash);
